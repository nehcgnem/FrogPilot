...
void update_model(UIState *s, FrogPilotUIState *fs,
  ...
  update_line_data(s, plan_position, frogpilot_toggles.value("model_ui").toBool() ? path_width : 0.9, 1.22, &scene.track_vertices, max_idx, false);
}
...
static void update_state(UIState *s, FrogPilotUIState *fs) {
  ...
  scene.started |= fs->frogpilot_scene.force_onroad;
