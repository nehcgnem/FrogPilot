...
void AnnotatedCameraWidget::updateState(const UIState &s, const FrogPilotUIState &fs) {
  ...
  if (frogpilot_toggles.value("show_speed_limits").toBool() || frogpilot_toggles.value("speed_limit_controller").toBool()) {
    speedLimit = fs.frogpilot_scene.speed_limit_overridden ? fs.frogpilot_scene.speed_limit_overridden_speed : fs.frogpilot_scene.speed_limit;
    if (!fs.frogpilot_scene.speed_limit_overridden && !frogpilot_toggles.value("show_speed_limit_offset").toBool()) {
      speedLimit += frogpilot_nvg->speedLimitOffset;
    }
  } else {
  ...
  // FrogPilot variables
  frogpilot_nvg->updateState(s, fs, frogpilot_toggles);
}

void AnnotatedCameraWidget::drawHud(QPainter &p, const FrogPilotUIState &fs, const QJsonObject &frogpilot_toggles) {
  p.save();
  ...
  if (has_us_speed_limit) {
    ...
    p.save();
    p.setOpacity(fs.frogpilot_scene.speed_limit_overridden ? 0.25 : 1.0);
    if (!fs.frogpilot_scene.speed_limit_overridden && frogpilot_toggles.value("show_speed_limit_offset").toBool()) {
  ...
  // current speed
  if (!(bigMapOpen || frogpilot_toggles.value("hide_speed").toBool())) {
...
void AnnotatedCameraWidget::paintGL() {
  ...
  if (s->scene.world_objects_visible) {
    update_model(s, fs, model, sm["uiPlan"].getUiPlan());
  ...
  drawHud(painter, *fs, frogpilot_toggles);
